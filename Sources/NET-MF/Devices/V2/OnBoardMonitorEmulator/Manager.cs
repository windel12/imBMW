...
namespace OnBoardMonitorEmulator
{
    public static class Manager
    {
        ...
        public static void Init(SerialPort port)
        {
            _port = port;
            _port.DataReceived += new SerialDataReceivedEventHandler(iBus_DataReceived);
        ...
        static void SendMessage(object o)
        {
            ...
            {
                port.Write((byte[])o);
                Thread.Sleep(port.AfterWriteDelay);
                return;
            }
            ...
            Thread.Sleep(m.AfterSendDelay > 0 ? m.AfterSendDelay : port.AfterWriteDelay); // Don't flood iBus
        }
